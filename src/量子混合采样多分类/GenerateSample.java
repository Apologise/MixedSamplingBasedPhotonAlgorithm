...
public class GenerateSample {
	...
	public void generateSample(Instance inst, Instances inputData, Instances majority, List<List<Instance>> output,
			int N) {
		...
		for (int i = 0; i < output.size(); ++i) {
			...
			while (IR != 0) {
				...
				for (int j = 0; j < inputData.numAttributes() - 1; ++j) {
					...
					/*
					 * if(gap<0.2) { gap = 0.2; }else if(gap >0.8) { gap = 0.8; }
					 */
				
						// 随机选择一个邻居
						double index = Math.random();
						int kth = (int) (index * setting.K);
						double diff = inputData.get(knn.get(kth)).value(j) - inst.value(j);
	...
	public List<Integer> calNeighborsWithDensity(Instance inst, Instances minority, Instances majority) {
		...
		/*
		 * System.out.print("["); for(int ii = 0; ii < knn.size(); ++ii) { if(ii== 0) {
		 * System.out.print(knn.get(ii)); }else { System.out.print(","+knn.get(ii)); } }
		 * System.out.print("],");
		 */
		return knn;
	}
	 ...
	 * 
	 * Instances systhetic = new Instances(inputData); systhetic.clear(); for(int i
	 * = 0; i < inputData.size(); ++i) { List<Integer> knn = kNeighbors(inputData,
	 * inputData.get(i)); int IR = N; while(IR != 0) { double[] values = new
	 * double[inputData.numAttributes()]; Random random = new Random(); int index =
	 * random.nextInt(SETTING.K); for(int j = 0; j < inputData.numAttributes()-1;
	 * ++j) { double gap = Math.random(); double diff =
	 * inputData.get(knn.get(index)).value(j)-inputData.get(i).value(j); values[j] =
	 * inputData.get(i).value(j) + gap*diff; } values[inputData.numAttributes()-1] =
	 * inputData.get(0).classValue(); systhetic.add(inputData.get(0).copy(values));
	 * IR--; } } return systhetic; }
	 */
